...
//        $this->warn('Slack Wipe: Make sure a worker is running on the queue.');
        ...
        if (count($channelsToWipe) > 0) {
            ...
            if ($response->ok) {
                ...
                foreach ($channels as $channel) {
                    $this->cleanupSingleChannel($scope, $channelsToWipe, $channel);
                }
            } else {
    ...
    private function cleanupSingleChannel($scope, $channelsToWipe, $channel)
    {
        foreach ($channelsToWipe as $channelToWipe) {
            if ($channelToWipe['name'] == $channel->name) {
                ...
                $this->dispatchCleanupJobs($scope, $channel->id, $channelToWipe);
            }
        }
    }

    private function dispatchCleanupJobs($scope, $channelId, $channel)
    {
        /*
            Other options
        ...
        */
        ...
        //todo implement paging or schedule run job more often per day and then it will start to catch-up: look at php generators in laravel

        if ($scope == self::SCOPE_PRIVATE) {
            $conversations = SlackGroup::history($channelId, $count, $latest);
        } else {
            $conversations = SlackChannel::history($channelId, $count, $latest);
        }
        ...
        if ($conversations->ok) {
            ...
            foreach ($conversations->messages as $message) {
                dispatch(new DeleteSlackMessage($channelId, $message))->onQueue(config('slackwipe.slack_wipe_queue', 'default'));
            ...
            $error = (isset($conversations->error)) ? ' ('.$conversations->error.')' : '';
            $message = 'Slack Wipe: An issue occurred while retrieving the conversations for the \''.$channel['name'].'\' ('.$channelId.') channel'.$error;
